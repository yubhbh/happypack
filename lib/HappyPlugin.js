...
HappyPlugin.prototype.apply = function(compiler) {
  ...
  engageWatchMode = fnOnce(function() {
    ...
    compiler.plugin('done', function() {
      that.state.foregroundWorker = createForegroundWorker(that, compiler);
    });
...
HappyPlugin.prototype.start = function(compiler, done) {
  ...
  async.series([
    function registerCompilerForRPCs(callback) {
      that.threadPool.getRPCHandler().registerActiveCompiler(compiler, that.config.compilerId);
    ...
    function launchAndConfigureThreads(callback) {
      that.threadPool.start(function() {
        ...
        that.threadPool.configure(that.config.compilerId, serializedOptions, callback);
      });
    },
...
function createForegroundWorker(plugin, compiler) {
  var loaders = plugin.state.loaders;
  var rpcHandler = plugin.threadPool.getRPCHandler();
  var fakeCompiler = new HappyFakeCompiler({
    id: 'foreground',
    compilerId: plugin.config.compilerId,
